...
module.exports = {
  ...
  updateAdminBanner: async (req, res) => {
    ...
    if (counter) {
      ...
      if (req.file) {
        const currentTime = Date.now();
        const { locations } = await uploadImages(req.file, `Banner/${currentTime}`);
        await removeFromS3(counter?.banner?.split('.com/')[1]);
        admin.banner = locations.banner[0];
      }
    }
    await ADMIN.findOneAndUpdate({ _id: id }, { $set: { ...admin } });

    return res.status(200).send({ code: 200, message: 'Banner updated', success: true });
  },
